public class Wordle {
    ...
    public static String chooseSecretWord(String[] dict) {
        ...
        // make a random index between 0  and dict.length
        int randomIndex = (int) (Math.random() * dict.length) ;

        // Select the word at  place
    ...
    public static boolean containsChar(String secret, char c) {
        ...
        // go over every letter is secret
		for (int i = 0 ; i < secret.length() ; i++)
        {
            // if char is secret return true
    ...
    public static void computeFeedback(String secret, String guess, char[] resultRow) {
        ...
        for (int i = 0 ; i < secret.length() ; i++)
        {
            ...
            // Exact match (same letter, same position)
            if (guess.charAt(i) == secret.charAt(i))
                resultRow[i]='G';

              // Letter appears somewhere else in the secret
            else if (containsChar(secret , guess.charAt(i)))
               resultRow[i]='Y';

              // Letter does not appear anywhere in the secret
            else
              resultRow[i]='_';
        }
    }
    ...
    public static boolean isAllGreen(char[] resultRow) {
        ...
        int countG = 0 ;
         ...
         // go over the result row
        for (int i = 0 ; i < 5 ; i++)
        {
            // checking if its G and if yes we add 1
            if (resultRow[i] == 'G')
                countG++;
        }
        if (countG == 5) // if its 5 we return true
            return true;
